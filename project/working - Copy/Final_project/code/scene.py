...
class Scene(tk.Frame):
    ...
    def __init__(self, filename, disk_robot, build_fn, master=None, resolution =500):
        ...
        # setup the GUI
        self.master.title("Hybrid A* algorithm")
        self.canvas = tk.Canvas(self.master, width=resolution, height=resolution, bg="white")
        #self.canvas.pack()
        self.bt_new = tk.Button(self.master, text="Random parallel parking space", command=self.random_query, state=tk.DISABLED)
        self.bt_default = tk.Button(self.master, text="perpendicular parking", command=self.default_query, state=tk.DISABLED)
        self.bt_search = tk.Button(self.master, text="Get Path", command=self.search, state=tk.DISABLED)
        self.bt_roadmap = tk.Button(self.master, text="Generate start point", command=self.generate)
    ...
    def loadProblem(self):
        ...
        try:
            ...
            # Here we have fixed our coordinates for a perpendicular parking case.
            self.default_start = (3, 5, math.radians(90 -90.0000001))

            self.default_goal = (36.1471127379512, 23.441343008942546, math.radians(90 - 9.999999406318238e-08))
    ...
    def random_query(self, event=None):
        ...
        # Parallel parking spots available:

        q_goal_tl = (25, 6, -89.9999999)
        q_start_tl = (3, 5, 90.0000001)
        q_goal_tr = (35, 6, -89.9999999)
        q_start_tr = (3, 5, 90.0000001)
        q_goal_b = (19, 36, 9.999999406318238e-08)
        q_start_b = (3, 5, 0.0000001)

        space_right_top = (q_start_tr, q_goal_tr)
        space_left_top = (q_start_tl, q_goal_tl)
        space_bottom = (q_start_b, q_goal_b)
        parallel_parking = [space_right_top, space_left_top, space_bottom]
        spots = random.choice(parallel_parking)
        self.start = (spots[0][0], spots[0][1], math.radians(90 - spots[0][2]))
        self.goal = (spots[1][0], spots[1][1], math.radians(90 - spots[1][2]))
        self.draw_config(self.start,"green","start")
        self.draw_config(self.goal,"blue","goal")
    ...
    def generate(self):
        ...
        if self.roadmap is None or len(self.roadmap.getVertices()) == 0:
            ...
            self.draw_roadmap(self.roadmap, 0.2)
    ...
    def draw_config(self, config, color, name):
        ...
        if self.disk_robot: 
            ...
            u_x = config[0] - self.scene_xmin + 0.45*sin(config[2])
            u_y = config[1] - self.scene_ymin + 0.45*cos(config[2])
            self.canvas.create_oval(world_scale * (u_x - radius), world_scale * (u_y - radius),
                                    world_scale * (u_x + radius), world_scale * (u_y + radius), fill=color, tag=name)
    ...
    def draw_path(self,path):
        ...
        # world_scale = self.resolution / self.scene_width
        # canvas.create_arc(coord, start=30, extent=120, style=tk.ARC, width=3)
        ...
        #     self.canvas.create_arc(world_scale * u_x, world_scale * u_y,
        #                             world_scale * (v.getConfiguration()[0] - self.scene_xmin),
        #                             world_scale * (v.getConfiguration()[1] - self.scene_ymin), fill="grey40",
        #                             dash=(1, 1), tag="roadmap", start= 0, extent=350, style=tk.ARC, width=1)
    ...
    def draw_roadmap(self, roadmap, radius=0.2):
        world_scale = self.resolution/self.scene_width
