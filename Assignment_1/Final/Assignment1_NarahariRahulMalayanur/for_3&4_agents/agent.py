...
class Agent(object):
    ...
    def computeNewVelocity(self, neighbors=[]):
        ...
        cf = []
        nlist =[]
        for k in neighbors:      # Calculating distances and agent velocities and appending
            ...
            if sensing_rad > self.dhor: # Calcluating the sensing radius to increase the agents velocity.
                self.vnew[:] = self.gvel[:]
                return   # If the above section runs, then do not compute the following.
            else:
                nlist.append(k)  # Creating the neighbors list which are only colliding.
        for agent in nlist:
