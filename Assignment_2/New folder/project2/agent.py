...
class Agent(object):
    ...
    def computeForces(self, neighbors=[]):
        ...
        f_mag = [0, 0]
        epsilon = 0.2
        for j in neighbors:  # Calculating distances and agent velocities and appending
            if not self.id == j.id:
                sensing_rad = sqrt(np.dot(self.pos - j.pos, self.pos - j.pos))
                if sensing_rad < self.dhor:
                    sensing_list.append()
            # if sensing_rad < self.dhor:  # Calcluating the sensing radius to increase the agents velocity.
        ...
        for var in range(len(v_rel)):
            ...
            v_rela = self.vel - v_rel[var]

            c = abs(np.dot(dist, dist) - (self.radius + self.radius) ** 2)
