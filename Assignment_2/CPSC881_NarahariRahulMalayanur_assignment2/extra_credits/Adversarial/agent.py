...
class Agent(object):
    ...
    def computeForces(self, neighbors=[]):
        ...
        for var in range(len(v_rel)):
            ...
            value = np.dot(dist,dist)
            v_rela = v_rel[var] - epsilon*(dist/sqrt(value))
            a = np.dot(v_rela, v_rela) - (epsilon) ** 2
            b = np.dot(dist, v_rela) - epsilon * self.radius
            ...
            if d <= 0:
                ...
                d = sqrt(d)
                tau = c/ (-b + d)# Least time of collision.
                if tau < 0:
                    tau = math.inf  # No collision = no force
                else:
                    power_f = dist + v_rela*tau
                    power_force = 2*2*(power_f / d)/tau**3
